...
screen file_slots(title):
    ...
    use game_menu(title):
        ...
        fixed:
            ...
            ## input (페이지 제목을 플레이어가 수정할 수 있음)이 세이브/로드 버
            ## 튼보다 먼저 엔터에 반응하도록 합니다.
            order_reverse True
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    style_prefix "check"
                    label _("스킵")
                    textbutton _("읽지 않은 텍스트까지 모두 스킵") action Preference("skip", "toggle")
                    textbutton _("선택지 이후에도 스킵") action Preference("after choices", "toggle")
                    textbutton _("화면 전환 효과를 모두 스킵") action InvertSelected(Preference("transitions", "toggle"))
